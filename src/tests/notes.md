...
- **Coverage denominator:** Exclude build hosts (`compiler/cli.ts`, `compiler/register.ts`, `compiler/ts.ts`) and the deprecated `core/metadata.ts` shim. Under Node, also exclude `bun-adapter` / `deno-adapter` (attributed via `unit-bun` / `runtime-deno` Codecov uploads). Runtime Node coverage is scoped to `node-adapter` so it does not inflate the project denominator.
...
- **Missing middleware (falsy instance):** `DIContainer.resolve` throws for unregistered tokens; to hit the `Middleware ${name} not registered` check, register `{ useValue: null }` so `getInjectable` returns falsy.
