  ...
  while (index < chars.length) {
    ...
    if (type) {
      ...
      if (index < chars.length && chars[index] === '(') {
        ...
        while (index < chars.length) {
          ...
          if (char === '\\') {
            if (index >= chars.length) {
              throw new PathError(`Unterminated escape sequence in pattern at index ${index - 1}`, str);
            }
            patternValue += char + chars[index++];
            continue;
          }
        ...
        if (count) {
          throw new PathError(`Unmatched "(" at index ${patternStart}`, str);
        }
        pattern = patternValue;
      }

      tokens.push({ type: "param", index: tokenStart, value: paramName, pattern });
    } else if (value === "*") {
 ...
 */
export function pathCompiler<P extends ParamData = ParamData>(
  path: Path,
...
export function pathMatcher<P extends ParamData>(
  ...
  const { decode = decodeURIComponent, delimiter = DEFAULT_DELIMITER } =
    options;
  const { regexp, keys } = pathToRE(path, options);
  ...
  return function match(input: string): Match<P> {
    ...
    return { path, params: params as P };
  };
}

export function pathToRE(
  path: Path | Path[],
  ...
  for (const input of pathsToArray(path, [])) {
    ...
    sources.push(toRegExpSource(data.tokens, delimiter, keys, data.originalPath));
  }
...
function toRegExpSource(
  ...
  for (const token of tokens) {
    ...
    if (token.type === "group") {
      const groupSource = toRegExpSource(token.tokens, delimiter, keys, originalPath);
      result += `(?:${groupSource})?`;
      backtrack = "";
      continue;
    }
    ...
    if (token.type === "param" || token.type === "wildcard") {
      ...
      if (token.type === "param") {
        const capture = token.pattern ? `(${token.pattern})` : `(${negate(delimiter, isSafeSegmentParam ? "" : backtrack)}+)`;
        result += capture;
      } else {
        result += `([\\s\\S]+)`;
      }
