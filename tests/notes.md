...
- **webergency-tsc plugins:** stock tsc ignores ts-patch `transform` plugins. `createBeforeTransformers` always runs `@webergency-utils/typechecker/transformer` (so `assert<T>()` is rewritten) then other listed before-transforms, then the server AOT plugin. Skip `after` / `afterDeclarations` and do not double-run the server plugin.
