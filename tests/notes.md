...
- **AOT comment leak:** Generated `__val_` / `__parse_` / `__ser_` trees keep template `pos` values. After they are spliced into a user file, tsc reprints nearby JSDoc / `//` comments inside those functions (`let parts = [ /** POST /webhooks… */ ]`, `typeof obj !== "object" || // comment`). Detach generated expressions (`pos: -1`, `EmitFlags.NoComments`) before prepend. Assert via `emitFiles` that distinctive comment text is absent from generated locals, not the whole file (method JSDoc is supposed to remain on the class).
